...
trait TreePrintOps {
  ...
  final private def v(tree: Desc, name: String)(implicit indent: Int): String = tree match {
    ...
    case DescSnoop(_, _, spec, fct) => s"desc snoop ${v(fct)}${v(spec)} $name"
    case DescPipeVar(_, _, spec) => s"desc pipeline ${v(spec)} $name"
    case DescPipeIn(_, _, hosts, fct) => s"desc in ${v(fct)}pipeline(${vs(hosts)}) $name"
    case DescPipeOut(_, _, hosts, fct, st) => s"desc out ${v(fct)}${v(st)}pipeline(${vs(hosts)}) $name"
  ...
  final private def v(tree: Decl, name: String)(implicit indent: Int): String = tree match {
    ...
    case DeclSnoop(_, spec) => s"decl snoop sync ready ${v(spec)} $name;"
