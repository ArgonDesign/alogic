    ...
    def v(node: Node): Unit = {
      if (callback(node))
        node match {
          ...
          case ExprArrIndex(name, index)                        => { v(name); index foreach v }
          case ExprVecIndex(ref, index)                         => { v(ref); index foreach v }
    ...
    def r[E <: Node](node: Node): E = {
      val v = cb(node) match {
        ...
        case None => node match {
          ...
          // Expressions
          case ExprArrIndex(name, index)          => ExprArrIndex(r[DottedName](name), index map r[Expr])
          case ExprVecIndex(ref, index)           => ExprVecIndex(r[Expr](ref), index map r[Expr])
          ...
          case WaitCall(name)                     => WaitCall(r[DottedName](name))
  ...
  def collect[E](pf: PartialFunction[Node, E]): List[E] = {
    ...
    def c(node: Node): List[E] = {
      ...
      val tail: List[E] = node match {
        ...
        case ExprArrIndex(name, index)                    => c(name) ::: (index flatMap c)
        case ExprVecIndex(ref, index)                     => c(ref) ::: (index flatMap c)
