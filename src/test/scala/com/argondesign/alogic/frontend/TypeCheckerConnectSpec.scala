...
final class TypeCheckerConnectSpec extends AnyFreeSpec with AlogicTest {
  ...
  "The Typer should check Connect usage" - {
    ...
    // TODO: Add back active logic checks in a later stage
    ...
    "port select" - {
      for {
        (expr, msg) <- List(
          ("a.b", ""),
          ("a.c.x", ""),
          ("a.d", "No port named 'd' on singleton instance 'a'"),
          ("a.d.x", "No port named 'd' on singleton instance 'a'"),
          ("a.N", "No port named 'N' on singleton instance 'a'")
        )
      } {
        expr in {
          typeCheck {
          ...
          if (msg.isEmpty) {
            ...
            cc.messages should have length 2
            cc.messages(0) should beThe[Error](msg)
            cc.messages(1) should beThe[Note]("'.*' is defined here")
          }
        }
      }
    }
