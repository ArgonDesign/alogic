...
final class ExprOpsSpec extends AnyFreeSpec with AlogicTest {
  ...
  "ExprOps" - {
    "infix operator methods should build the appropriate binary expression node" - {
      "Expr rhs" - {
        "*" in {
          Expr(1) * Expr(2) shouldBe ExprBinary(Expr(1), "*", Expr(2))
        }
        "/" in {
          Expr(1) / Expr(2) shouldBe ExprBinary(Expr(1), "/", Expr(2))
        }
      ...
      "unsized lhs, Int rhs" - {
        "*" in {
          TypeAssigner(Expr(1)) * 2 shouldBe ExprBinary(Expr(1), "*", Expr(2))
        }
        "/" in {
          TypeAssigner(Expr(1)) / 2 shouldBe ExprBinary(Expr(1), "/", Expr(2))
        }
      ...
      "sized lhs, Int rhs" - {
        // format: off
        "*" in {
          TypeAssigner(ExprInt(false, 8, 1)) * 2 shouldBe ExprBinary(ExprInt(false, 8, 1), "*", ExprInt(false, 8, 2))
        }
        "/" in {
          TypeAssigner(ExprInt(false, 8, 1)) / 2 shouldBe ExprBinary(ExprInt(false, 8, 1), "/", ExprInt(false, 8, 2))
        }
        ...
        "<<" in {
          TypeAssigner(ExprInt(false, 8, 1)) << 2 shouldBe ExprBinary(ExprInt(false, 8, 1), "<<", Expr(2))
        }
        ">>" in {
          TypeAssigner(ExprInt(false, 8, 1)) >> 2 shouldBe ExprBinary(ExprInt(false, 8, 1), ">>", Expr(2))
        }
    ...
    "unary operator methods should build the appropriate Unary expression node" - {
      ...
      "-" in {
        -Expr(1) shouldBe ExprUnary("-", Expr(1))
      }
      "~" in {
        ~Expr(1) shouldBe ExprUnary("~", Expr(1))
      }
    ...
    "other combiner methods" - {
      ...
      "call Expr" in {
        Expr(1) call Expr(2) shouldBe {
          ExprCall(Expr(1), List(ArgP(Expr(2))))
        }
      }
